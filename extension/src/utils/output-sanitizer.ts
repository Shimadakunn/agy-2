...
const SENSITIVE_KEY_MARKERS = [
  ...
  'bearer', 'token', 'accesstoken', 'refreshtoken', 'idtoken',
  'password', 'passwd', 'pwd', 'secret', 'clientsecret',
  'apikey', 'session', 'sessionid', 'sid', 'csrf', 'xsrf',
  'credential', 'privatekey', 'accesskey', 'auth', 'oauth',
] as const;
...
export function sanitizeAndLimitOutput(
  ...
  const { value: sanitizedValue, redacted } = sanitizeValue(value, {
    maxDepth, maxArrayLength, maxObjectKeys, maxStringLength,
  });
